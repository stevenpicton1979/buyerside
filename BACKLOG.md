...
- [x] Audit every data field displayed in `scout-report.html` and all contributing API routes. Classify each field as: (a) live from ZoneIQ, (b) live from Domain sandbox, (c) hardcoded mock for Chelmer demo, or (d) already a smart stub. Write the full audit as a comment block at the top of `property-lookup.js`.
...
- [x] Build a static suburb median lookup table as a JS object in `property-lookup.js` — keyed by Brisbane suburb name, value is approximate median house price. Manually curate the top 100 Brisbane suburbs by property search volume using publicly available ABS/REA data. Zero per-call cost. Add a comment: `// UPDATE QUARTERLY — source: ABS / REA public data`. Use this table to populate suburb median in the Scout Report, falling back to null if the suburb is not in the table.
- [x] Replace hardcoded comparable sales section with a deliberately incomplete stub — display 3 placeholder rows with blurred/redacted prices and the label "Comparable sales — included in your free Scout Report." Style as a teaser that creates curiosity, not an error state. Note: comparables are free per the brief but withheld at Stage 1 to drive email gate conversion.
- [x] Replace hardcoded property estimate with a teaser: "Estimated value range — unlocked in your Scout Report." Styled as a locked section, consistent with other teaser elements.
- [x] Replace hardcoded demand/supply meters with values derived from the static suburb lookup table where possible (e.g. derive demand signal from days-on-market data if available). Where not available, show neutral 50/50 with label "Based on current suburb data."
- [x] Verify Nominatim autocomplete is returning results for real Brisbane QLD addresses. Test with at least 3 addresses. If broken, fix. Do not replace with Google Geocoding — stays on Nominatim until Domain API approval.
- [x] Update DECISIONS.md: document static suburb lookup table approach and stub strategy.
- [x] Commit: `Sprint 5: Live ZoneIQ data, suburb lookup table, smart stubs throughout Scout Report`
